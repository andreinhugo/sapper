...
var transitiveDeps = _interopDefault(require('rollup-dependency-tree'));
...
function generate_client_manifest(
	...
	return `
		...
		import * as root_comp from '${__chunk_2.stringify(get_file(path_to_routes, manifest_data.root), false)}';
		export { root_comp };
...
function generate_server_manifest(
	...
	const imports = [].concat(
		...
		manifest_data.components.map((component, i) =>
			`import * as component_${i} from ${__chunk_2.stringify(get_file(path_to_routes, component))};`),
		`import * as root_comp from ${__chunk_2.stringify(get_file(path_to_routes, manifest_data.root))};`,
	...
	const build_dir = __chunk_2.posixify(path.normalize(path.relative(cwd, dest)));
	const src_dir = __chunk_2.posixify(path.normalize(path.relative(cwd, src)));
...
function extract_css(
 ...
 = {
	...
	components.forEach(component => {
		const resolved = __chunk_2.normalize_path(path.resolve(dirs.routes, component.file));
...
class RollupResult  {
	...
	constructor(duration, compiler, sourcemap) {
		...
		this.chunks = compiler.chunks.map(chunk => ({
			...
			modules: Object.keys(chunk.modules).map(m => __chunk_2.normalize_path(m))
		}));
...
function handleError(err, recover = false) {
	...
	stderr(__chunk_2.kleur.bold().red(`[!] ${__chunk_2.kleur.bold(message.toString())}`));

	if (err.url) {
		stderr(__chunk_2.kleur.cyan(err.url));
	}
	...
	if (err.frame) {
		stderr(__chunk_2.kleur.dim(err.frame));
	}

	if (err.stack) {
		stderr(__chunk_2.kleur.dim(err.stack));
	}
