...
async function findNotPendingIndex(list) {
...
async function filterNotPending(list) {
	const states = await Promise.all(list.map(p => promiseState(p)));
...
function exportQueue({ concurrent, handleFetch, handleResponse, fetchOpts, callbacks } ) {
	const urls = [];
	let fetching  = [];
	let saving  = [];

	function addToQueue(p, queue) {
...
async function _export({
	...
	timeout = 5000,
	concurrent = 8,
	oninfo = __chunk_5.noop,
	onfile = __chunk_5.noop,
	entry = '/'
} = {}) {
	...
	__chunk_5.rimraf(export_dir);

	__chunk_5.copy(static_files, export_dir);
	__chunk_5.copy(path.join(build_dir, 'client'), path.join(export_dir, 'client'));
	__chunk_5.copy(path.join(build_dir, 'service-worker.js'), path.join(export_dir, 'service-worker.js'));
	__chunk_5.copy(path.join(build_dir, 'service-worker.js.map'), path.join(export_dir, 'service-worker.js.map'));

	const defaultPort = process.env.PORT ? parseInt(process.env.PORT) : 3000;
	const port = await __chunk_1.find(defaultPort);
	...
	function save(url, status, type, body) {
		...
		if (is_html) {
			...
			body = __chunk_6.minify_html(body);
		}
		...
		__chunk_5.mkdirp(path.dirname(export_file));
	...
	async function handleFetch(url, { timeout, host, host_header }) {
		const href = url.href;
		const timeout_deferred = new __chunk_1.Deferred();
	...
	async function handleResponse(fetched, fetchOpts) {
		...
		if (range === 2 && type === 'text/html') {
			...
			let link = httpLinkHeader.parse(response.headers.get('Link') || '');
